...
class NonkycExchange(ExchangePyBase):
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        side_str = (
            CONSTANTS.SIDE_BUY if trade_type is TradeType.BUY else CONSTANTS.SIDE_SELL
        )
        ...
        try:
            ...
            is_server_overloaded = (
                ...
                and "Unknown error, please check your request or try again later."
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                if event_type == "report":
                    ...
                    if reportType == "trade":
                        ...
                        if tracked_order is not None:
                            fee = TradeFeeBase.new_spot_fee(
                                ...
                                flat_fees=[
                                    TokenAmount(
                                        amount=Decimal(message_params.get("tradeFee")),
                                        token=quote_asset,
                                    )
                                ],
                            )
                            trade_update = TradeUpdate(
                                ...
                                fill_base_amount=Decimal(
                                    message_params["executedQuantity"]
                                ),
                                fill_quote_amount=Decimal(
                                    message_params["executedQuantity"]
                                )
                                * Decimal(message_params["price"]),
                    ...
                    total_balance = Decimal(balance_entry["available"]) + Decimal(
                        balance_entry["held"]
                    )
    ...
    async def _update_order_fills_from_trades(self):
        ...
        """
        small_interval_last_tick = (
            self._last_poll_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL
        )
        small_interval_current_tick = (
            self.current_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL
        )
        ...
        if long_interval_current_tick > long_interval_last_tick or (
            ...
            for trading_pair in trading_pairs:
                ...
                tasks.append(
                    self._api_get(
                        path_url=url_path, params=params, is_auth_required=True
                    )
                )
                ...
                for trade in trades:
                    ...
                    if exchange_order_id in order_by_exchange_id_map:
                        ...
                        fee = TradeFeeBase.new_spot_fee(
                            ...
                            flat_fees=[
                                TokenAmount(
                                    amount=Decimal(trade["fee"]), token=quote_asset
                                )
                            ],
                        )
                        trade_update = TradeUpdate(
                        ...
                        self.trigger_event(
                            ...
                            OrderFilledEvent(
                                ...
                                order_type=(
                                    ...
                                    if trade["side"] != trade["triggeredBy"]
                                ...
                                trade_fee=DeductedFromReturnsTradeFee(
                                    flat_fees=[
                                        TokenAmount(quote_asset, Decimal(trade["fee"]))
                                    ]
                                ),
                                exchange_trade_id=str(trade["id"]),
                            ),
        ...
        if order.exchange_order_id is not None:
            ...
            for trade in filtered_trades:
                ...
                fee = TradeFeeBase.new_spot_fee(
                    ...
                    percent_token=quote_asset,
                    flat_fees=[
                        TokenAmount(amount=Decimal(trade["fee"]), token=quote_asset)
                    ],
                )
                trade_update = TradeUpdate(
    ...
    async def _update_balances(self):
        ...
        balances = await self._api_get(
            path_url=CONSTANTS.USER_BALANCES_PATH_URL, is_auth_required=True
        )
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(
        ...
        for symbol_data in filter(nonkyc_utils.is_market_active, exchange_info):
            ...
            mapping[symbol] = combine_to_hb_trading_pair(
                base=symbol_data["primaryTicker"], quote=symbol.split("/")[1]
            )
